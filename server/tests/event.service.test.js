jest.mock("@prisma/client", () => {
  ...
  describe("Event Service", () => {
    ...
    describe("updateEventSeats", () => {
      it("should update event seats with action add", async () => {
        const event = {
          totalSeats: ["A1", "A2"],
          totalTicketsSold: "2",
        };
        eventMock.findUnique.mockResolvedValue(event);
        eventMock.update.mockResolvedValue({ totalSeats: ["A1", "A2", "A3"], totalTicketsSold: "3" });
        const result = await updateEventSeats("e1", ["A3"], 1, "add");
        expect(eventMock.findUnique).toHaveBeenCalledWith({
          where: { id: "e1" },
        ...
        expect(result).toEqual({ totalSeats: ["A1", "A2", "A3"], totalTicketsSold: "3" });
      });
  
      it("should update event seats with action remove", async () => {
        const event = {
          totalSeats: ["A1", "A2", "A3"],
          totalTicketsSold: "3",
        };
        eventMock.findUnique.mockResolvedValue(event);
        eventMock.update.mockResolvedValue({ totalSeats: ["A1", "A3"], totalTicketsSold: "2" });
        const result = await updateEventSeats("e1", ["A2"], 1, "remove");
        expect(eventMock.findUnique).toHaveBeenCalledWith({
          where: { id: "e1" },
          select: { totalSeats: true, totalTicketsSold: true },
        });
        ...
        expect(result).toEqual({ totalSeats: ["A1", "A3"], totalTicketsSold: "2" });
      });
      ...
      it("should throw error for invalid action", async () => {
        ...
        eventMock.findUnique.mockResolvedValue(event);
        await expect(updateEventSeats("e1", ["A3"], 1, "invalid")).rejects.toThrow("Invalid action");
      });
    });
  });
