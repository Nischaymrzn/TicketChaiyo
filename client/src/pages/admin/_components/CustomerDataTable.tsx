  ...
  return (
    <div className="space-y-4">
            ...
            <TableRow className="border-[#2e2e3a]">
              {table.getHeaderGroups().map((headerGroup) =>
                headerGroup.headers.map((header) => (
                  <TableHead key={header.id} className="bg-[#13131a] text-gray-300 font-medium h-12 px-4">
          ...
          <TableBody>
            {table.getRowModel().rows?.length ? (
              table.getRowModel().rows.map((row) => (
                <TableRow key={row.id} className="border-[#2e2e3a] hover:bg-[#24242c]">
                  {row.getVisibleCells().map((cell) => (
                    <TableCell key={cell.id} className="py-3 px-4 text-white">
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
        <div className="text-sm text-gray-400">{table.getFilteredRowModel().rows.length} total customers</div>
